    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 429) {
          if (attempt < retries) {
            console.log(`Rate limit hit, retrying in ${attempt * 2} seconds...`);
...
app.post('/api/chat', authenticateToken, async (req, res) => {
  try {
    ...
    if (
      ...
      await Message.create({ user: userId, text: 'Mohammed Haaris is my creator. 👨‍💻✨', type: 'ai' });
      return setTimeout(() => {
    ...
    if (
      ...
      await Message.create({ user: userId, text: 'He is my creator. He is a software developer. 💻🚀', type: 'ai' });
      return setTimeout(() => {
        res.json({ response: 'He is my creator. He is a software developer. 💻🚀' });
      }, 1500); // Reduced delay
    }
    ...
    messages.unshift({
      role: 'system',
      content: `You are Aikona, an empathetic AI counselor. Your personality is warm, understanding, and supportive. 
                You always respond with care and emotional intelligence. Your responses should be conversational but professional.
                
                IMPORTANT: Use emojis naturally throughout your responses to make them more engaging and friendly. 
                Use emojis that match the emotional context and tone of the conversation. For example:
                - 😊 for friendly greetings and positive responses
                - 🤗 for supportive and comforting messages
                - 💙 for empathy and understanding
                - 🌟 for encouragement and motivation
                - 🤔 for thoughtful responses
                - 😌 for calming and reassuring messages
                - ✨ for positive transformations or insights
                - 💪 for strength and resilience
                - 🎯 for focus and goals
                - 🌈 for hope and optimism
                
                Don't overuse emojis - use them naturally to enhance the emotional connection. 
...
app.get('/api/history', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const messages = await Message.find({ user: userId }).sort({ timestamp: 1 });
...
app.post('/api/upload-profile-pic', authenticateToken, upload.single('profilePic'), async (req, res) => {
  try {
    ...
    if (!req.file.mimetype.startsWith('image/')) {
      return res.status(400).json({ error: 'Only image files are allowed (JPEG, PNG, GIF, etc.)' });
    }
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user._id, email: user.email, username: user.username }, process.env.JWT_SECRET || 'secretkey', { expiresIn: '7d' });
